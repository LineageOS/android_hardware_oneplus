...
class KeyHandler(context: Context) : DeviceKeyHandler {
    ...
    override fun handleKeyEvent(event: KeyEvent): KeyEvent {
        if (event.action == KeyEvent.ACTION_DOWN) {
            when (event.scanCode) {
                POSITION_TOP -> {
                    val mode = sharedPreferences.getString(NOTIF_SLIDER_TOP_KEY, "2")!!.toInt()
                    audioManager.setRingerModeInternal(mode)
                    vibrateIfNeeded(mode)
                }
                POSITION_MIDDLE -> {
                    val mode = sharedPreferences.getString(NOTIF_SLIDER_MIDDLE_KEY, "1")!!.toInt()
                    audioManager.setRingerModeInternal(mode)
                    vibrateIfNeeded(mode)
                }
                POSITION_BOTTOM -> {
                    val mode = sharedPreferences.getString(NOTIF_SLIDER_BOTTOM_KEY, "0")!!.toInt()
                    audioManager.setRingerModeInternal(mode)
                    vibrateIfNeeded(mode)
                }
            }
