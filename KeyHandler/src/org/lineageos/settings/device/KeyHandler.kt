...
class KeyHandler(context: Context) : DeviceKeyHandler {
    ...
    private val sharedPreferences = packageContext.getSharedPreferences(
    ...
    private val broadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            if (intent?.getBooleanExtra(AudioManager.EXTRA_STREAM_VOLUME_MUTED, false) == false) {
                wasMuted = false
            }
        }
    }
    ...
    override fun handleKeyEvent(event: KeyEvent): KeyEvent {
        if (event.action == KeyEvent.ACTION_DOWN) {
            when (event.scanCode) {
                POSITION_TOP -> {
                    handleMode(sharedPreferences.getString(NOTIF_SLIDER_TOP_KEY, "2")!!.toInt())
                }
                POSITION_MIDDLE -> {
                    handleMode(sharedPreferences.getString(NOTIF_SLIDER_MIDDLE_KEY, "1")!!.toInt())
                }
                POSITION_BOTTOM -> {
                    handleMode(sharedPreferences.getString(NOTIF_SLIDER_BOTTOM_KEY, "0")!!.toInt())
                }
            }
        }
        return event
    ...
    private fun handleMode(mode: Int) {
        ...
        when (mode) {
            ...
            AudioManager.RINGER_MODE_VIBRATE, AudioManager.RINGER_MODE_NORMAL -> {
                notificationManager.setZenMode(Settings.Global.ZEN_MODE_OFF, null, TAG)
                audioManager.setRingerModeInternal(mode)
                if (muteMedia && wasMuted) audioManager.adjustVolume(AudioManager.ADJUST_UNMUTE, 0)
            }
